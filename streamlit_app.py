...
st.set_page_config(page_title="Extended Kalman Filter for Macro Beta", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📊 EKF Macro Beta Tracking Engine</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Time‑varying sensitivity of ETF returns to macro shocks</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 EKF Beta")
...
st.sidebar.markdown(f"**Primary macro:** {config.PRIMARY_MACRO} | **Q:** {config.PROCESS_NOISE}, **R:** {config.MEASUREMENT_NOISE}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'ekf_beta_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Macro beta: {etf['ekf_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Beta"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Beta", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("📊 Top ETFs by Macro Beta (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Extended Kalman filter** estimates a time‑varying beta coefficient: ETF return = β_t * macro_t + noise.
        - β_t evolves as a random walk (state). The filter updates β_t sequentially.
        - The score is the final β estimate for the primary macro (e.g., VIX).
        - Positive β → ETF tends to rise when the macro increases (e.g., VIX up → risk‑on? depends on ETF).
        - High absolute β → ETF is highly sensitive to macro shocks.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the beta estimates.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Extended Kalman Filter | Time‑varying macro beta")
